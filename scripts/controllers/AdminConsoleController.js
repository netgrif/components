define(['angular', '../modules/Admin'], function (angular) {
    angular.module('ngAdmin').controller('AdminConsoleController', ['$log', '$scope', '$http', '$snackbar', '$timeout', '$user', '$i18n', '$location', '$auth', '$config', '$process',
                ...
                $http.get("/api/group/all").then(function (response) {
                    response.$request().$get("groups").then(function (resources) {
                        ...
                        self.groups.forEach(org => {
                            org.add = function () {
                                if (!self.invitedUser.groups.find(g => g.name === this.name))
