...
export class Md2MonthViewComponent implements AfterContentInit {
    ...
    private _init() {
        ...
        const firstOfMonth = this._util.createDate(this._util.getYear(this.activeDate),
    ...
    private _createWeekCells() {
        ...
        for (let i = 0, cell = this._firstWeekOffset; i < daysInMonth; i++ , cell++) {
            ...
            const ariaLabel = this._locale.format(date, this._dateFormats.display.dateA11yLabel);
            const newWeek = this._util.getWeekOfYear(date);
            const offset = cell % 7;
            if (offset === 0 && i > 0) {
                this._weeks.push([]);
            }
