...
export class DateUtil {
    ...
    parseDate(date: string, fmt: string) {
        ...
        const parser = new RegExp(format.replace(/(.)(\1*)(?:\[([^\]]*)\])?/g,
        ...
        for (let i = 1; i < reIndex; i++) {
            ...
            if (this.isList(indexEntry)) { // for a, n or N
                ...
                const listValue = this.find(valList,
                    (v: any, index: any) => {
        ...
        const d = new Date(ctorArgs[0] || 0, ctorArgs[1] || 0, ctorArgs[2] || 0, ctorArgs[3] || 0, ctorArgs[4] || 0,
    ...
    createDate(year: number, month: number, date: number,
        ...
        const result = this._createDateWithOverflow(year, month, date, hours, minutes, seconds);
    ...
    getFirstDateOfWeek(date: Date, firstDayOfWeek: number) {
        const day: number = date.getDate() - ((7 + date.getDay() - firstDayOfWeek) % 7);
    ...
    incrementMonths(date: Date, numberOfMonths: number) {
        ...
        const dateInTargetMonth = new Date(date.getFullYear(), date.getMonth() + numberOfMonths, 1,
            date.getHours(), date.getMinutes());
    ...
    isDateWithinRange(date: Date, minDate: Date, maxDate: Date) {
        ...
        const minDateAtMidnight = this.isValidDate(minDate) ? this.createDateAtMidnight(minDate) : null;
        const maxDateAtMidnight = this.isValidDate(maxDate) ? this.createDateAtMidnight(maxDate) : null;
