...
export class Md2Clock implements AfterContentInit {
    ...
    get _hand(): any {
        ...
        if (this._hourView) {
            const outer = this.activeDate.getHours() > 0 && this.activeDate.getHours() < 13;
    ...
    private _init() {
        ...
        if (this.twelvehour) {
            for (let i = 1; i < (hourNames.length / 2) + 1; i++) {
                ...
                const enabled = this._util.isFullDateWithinRange(date, this.min, this.max);
                this._hours.push({
                    value: i,
    ...
    private setTime(event: any) {
        ...
        const outer = this._hourView && z > ((width * (CLOCK_OUTER_RADIUS / 100)) +
