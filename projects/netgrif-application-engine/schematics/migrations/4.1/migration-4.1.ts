...
import {fileEntryToTsSource, findNodesInChildren, forEachProjectFile} from '../../_utility/utility-functions';
        ...
        forEachProjectFile(tree, (file: FileEntry) => {
            if (file.path.endsWith('.ts') && !file.path.endsWith('.spec.ts')) {
                ...
                for (const node of extendsKeywords) {
                    const identifiers = findNodesInChildren(node, ts.SyntaxKind.Identifier, true);
                    if (identifiers.some(identifier => identifier.getText() === 'AbstractCaseView' || identifier.getText() === 'TabbedCaseView')) {
