...
export abstract class AbstractBreadcrumbsComponent implements OnDestroy, AfterViewInit {
    ...
    public resolveBreadcrumbs() {
        ...
        this._caseResourceService.searchCases(SimpleFilter.fromCaseQuery(searchBody), httpParams).pipe(take(1)).subscribe(result => {
            ...
            const filterCaseIndex = cases.findIndex(c => Array.from(c.tasks.values()).some(t => t.taskStringId === filterId) && !fullPath.includes(this.immediateValue(c, AbstractBreadcrumbsComponent.NODE_PATH)));
            if (filterCaseIndex >= 0) {
                const filterCase = cases.splice(cases.findIndex(c => Array.from(c.tasks.values()).some(t => t.taskStringId === filterId) && !fullPath.includes(this.immediateValue(c, AbstractBreadcrumbsComponent.NODE_PATH))), 1)[0];
            ...
            if (this.redirectOnClick) {
                cases.forEach(c => this.redirectUrls.set(this.immediateValue(c, AbstractBreadcrumbsComponent.NODE_PATH), [this._dynamicRoutingService.route, c.tasks?.get(AbstractBreadcrumbsComponent.ITEM_SETTINGS).taskStringId]))
            }
