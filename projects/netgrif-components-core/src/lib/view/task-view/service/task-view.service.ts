...
export class TaskViewService extends AbstractSortableViewComponent implements OnDestroy {
    ...
    public loadPage(requestContext: PageLoadRequestContext): Observable<TaskPageLoadRequestResult> {
        ...
        if (this._userService.user.isAnonymous()) {
            const caseIds = Array.isArray(requestContext.filter.getRequestBody()['case']) ? requestContext.filter.getRequestBody()['case'].map(aCase => aCase.id) : [requestContext.filter.getRequestBody()['case'].id];
