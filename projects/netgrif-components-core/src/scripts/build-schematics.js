...
// run the compiler
exec(path.join('node_modules', '.bin', 'tsc -p projects', 'netgrif-components-core', 'tsconfig.schematics.json'), (error, stdout, stderr) => {
    if (stdout) {
        console.log(stdout);
    }
    if (error) {
...
function copySchematicFiles() {
    const sourcePrefix = path.join('projects', 'netgrif-components-core', 'schematics');
    const destinationPrefix = path.join('dist', 'netgrif-components-core', 'schematics');
